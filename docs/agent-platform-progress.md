...
2026-06-07 已启动 Spring AI / LangChain4j adapter Spike，新增 Java 17 profile `adapters-java17`，当前覆盖 `TEXT_CHAT` / `TEXT_STREAM` / Tool schema 下发 / ToolCall 响应映射。
...
Spring AI / LangChain4j adapter Spike 使用 Java 17 profile 隔离，不进入默认 Java 8 主 reactor
...
完成 agent-model-provider-langchain4j Java 17 profile 下的 TEXT_CHAT / TEXT_STREAM / Tool schema 下发 / ToolCall 响应映射 Spike
完成 agent-model-provider-spring-ai Java 17 profile 下的 TEXT_CHAT / TEXT_STREAM / Tool schema 下发 / ToolCall 响应映射 Spike
```
...
近期不新增模型适配
不继续扩大自研模型协议面
Spring AI / LangChain4j adapter Spike 已在 Java 17 profile 下启动，但不进入默认 Java 8 主链路
