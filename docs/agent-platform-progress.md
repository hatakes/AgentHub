...
当前处于第一版 MVP 核心闭环已完成、MVP 接入说明和验收清单已完成、示例业务只读 Tool 试点已完成、MCP Adapter 最小设计已完成、真实业务系统接入验收材料已完成、同级独立最小业务接入样板已验收、等待真实生产业务系统接入阶段。
...
2026-06-04 已完成 P0 DS 默认模型配置固化、P0 Tool 选择约束增强、P1 错误恢复和边界测试、P2 JSON Schema Structured Output 基础支持。当前 64 个测试全部通过。
...
2026-06-15 已同步 `agent-document-processing` 接口 DTO 分层：`AnalyzeAttachmentRequest`、`AttachmentAnalysisResponse`、`DocumentOutlineResponse` 归入 domain，api 层保持 HTTP 入口职责。
2026-06-15 已新增项目上手总入口 `docs/AgentHub_学习与落地计划.md`，说明当前项目由 Codex 持续推进后的真实状态、快速上手路径和二阶段学习路线。
...
Spring AI / LangChain4j adapter Spike 使用 JDK 17+ 自动激活 profile 隔离，不进入 Java 8 主 reactor
MCP 保留为生态互通协议方向，后续优先评估 MCP Java SDK / Spring AI MCP
智能附件分析可以放在本仓库作为业务样板模块，但必须通过 starter 接入 AgentHub
附件上传、OCR、文件解析、文档类型识别、字段抽取和业务规则判断属于业务模块职责，不下沉到 agent-core
确定性判断优先由业务代码完成，LLM 负责分类辅助、归纳总结和审核意见生成
```
...
已有 agent-model-provider-langchain4j Java 17 Spike 模块
已有 agent-model-provider-spring-ai Java 17 Spike 模块
已有 agent-core 单元测试
已有 agent-model-provider-http mock HTTP 协议测试
已有 agent-mcp-adapter 单元测试
已有 agent-model-provider-langchain4j Java 17 Spike 单元测试
...
已有 docs/plans/AgentHub_第一周日计划.md 和 xlsx 第一周执行计划
...
完成 agent-core 核心抽象和 DefaultAgentRuntime 设计型注释补强
...
完成最小业务样板接入验收记录：`docs/agenthub-business-acceptance-agent-business-minimal-demo.md`
...
P2 Spring AI / LangChain4j adapter Spike（JDK 17+ profile 下已启动 TEXT_CHAT / TEXT_STREAM / Tool schema 下发 / ToolCall 响应映射）
...
当前 Java 17 adapter Spike 只声明 `TEXT_CHAT` / `TEXT_STREAM` 能力；已支持向框架下发 Tool schema，并支持框架 ToolCall 响应映射为 AgentHub `ToolCall`。仍不声明完整 `TOOL_CALL` 能力，也不处理 Tool 结果消息或 Structured Output，避免绕过 AgentHub 既有权限和审计边界。
...
AgentExampleApplication 预算余额 Tool 流式 SSE 集成测试
```
...
新增智能附件分析业务样板后的 Java 8 主 reactor 验证：
...
建议下一步从“补模型协议”转为“生产业务接入验收 + 二阶段方向选择”。
...
近期不新增模型适配
不继续扩大自研模型协议面
Spring AI / LangChain4j adapter Spike 已在 JDK 17+ profile 下启动，但不进入 Java 8 主链路
Hutool AI adapter Spike 暂缓，不进入主链路
...
```text
1. 选择一个生产业务系统按 `docs/agenthub-starter-integration.md` 接入
2. 复用 `docs/agenthub-business-acceptance-record.md` 生成生产业务验收记录
3. 对比样板验收记录 `docs/agenthub-business-acceptance-agent-business-minimal-demo.md`
4. 根据生产接入结果决定是否补 Tool Schema、AuditEvent 或 PermissionResult 字段
5. 根据真实接入结果按 `docs/agenthub-phase2-decision.md` 决定是否评估 MCP Java SDK / Spring AI MCP、Gateway 或 Admin UI
```
