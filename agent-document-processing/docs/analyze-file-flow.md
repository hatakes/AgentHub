...
```text
userId  text  当前模块 中必须是 attachment-reviewer 才有分析权限
...
userId  text  当前模块 中必须是 attachment-reviewer
...
3. 当前 ModelProvider 仍是 mock 规则模型，不是真实 LLM；本模块 真实 AI 目前接在图片解析/OCR 层。
4. analysis 结果目前由规则模型直接计算，后续应沉淀更清晰的领域服务。
